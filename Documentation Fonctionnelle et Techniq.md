...
- Compteurs : nombre d'imports, de biens actifs, de lots, d'entrées d'historique, et de **biens validés (suivi par local)** — nombre de biens **distincts** de l'inventaire ayant fait l'objet d'au moins une mise à jour explicite (statut et/ou commentaire) via le Suivi de l'inventaire par local (§2.13), tous locaux confondus ; la simple création automatique d'une ligne à l'affichage d'un local (statut « Présent » par défaut, non modifié) ne compte pas comme une validation.
...
- **Bien ID coloré selon le dernier statut validé au Suivi par local** (§2.13) : vert si **Présent**, orange si **En Trop**, rouge si **Absent** ; couleur de police normale tant qu'aucune ligne n'a été **explicitement enregistrée** pour ce bien (la création automatique d'une ligne à l'affichage d'un local, statut par défaut non modifié, ne compte pas). Si le bien a plusieurs lignes de suivi (ex. déplacé de local), la plus récemment enregistrée fait foi.
...
- Actions sur la sélection :
  ...
  - **Étiquettes (PDF)** (`POST /assets/print-labels`, nouvel onglet) : page d'impression d'étiquettes au format ruban **90 x 36 mm, une étiquette par page** (`@page { size: 90mm 36mm; }`, marge intérieure de 4 mm sur les bords, un espace de 0,3 mm entre chaque ligne), générée comme PDF via l'impression navigateur (bouton « Imprimer », `window.print()` — même principe que l'export PDF des lots, §2.5), sans dépendance PDF côté serveur. Chaque étiquette affiche, dans cet ordre :
    ...
    Les tailles de police (lettrine 10 mm, reste de la destination 7,5 mm, ligne étage/bureau/code 3 mm, numéro de Bien ID 6 mm) sont calibrées pour rester lisibles tout en tenant dans les 90 x 36 mm ; la ligne étage/bureau/code conserve une hauteur figée à exactement 2 lignes (`max-height`, non rétrécissable) afin d'éviter tout chevauchement avec le code-barres — en cas de bureau très long, elle peut être tronquée au-delà de 2 lignes (jamais le code-barres ni son numéro, prioritaires) — à ajuster si besoin après un premier essai sur l'imprimante réelle.
...
- **Tableau des écarts** : liste les biens présents à la fois dans l'inventaire et dans un import GLPI dont le numéro local diffère du numéro de la pièce GLPI, avec Bien ID, désignation, numéro local actuel (survolé, une infobulle affiche la désignation du local), statut actif/exclu, **type GLPI** (Ordinateurs / Moniteurs / Périphériques / Logiciels / Imprimantes — le type de bien de l'import dont provient la ligne), lieu GLPI, numéro de la pièce GLPI et statut GLPI.
  ...
  - **Pagination** : en bas du tableau, une liste déroulante « Biens affichés » (`?page_size=`, 50/100/250, 50 par défaut) choisit le nombre de lignes par page, avec des boutons « Précédent »/« Suivant » et l'indicateur « Page X / Y » (`?page=`). Une page hors limites est automatiquement ramenée à la première ou dernière page valide.
- **Sélection multiple** des lignes (case à cocher par ligne + « tout sélectionner »), limitée aux lignes de la page affichée. Le nombre de lignes sélectionnées sur le nombre total (toutes pages confondues) s'affiche à côté du titre du tableau (ex. « 2 / 5 sélectionné(s) »).
...
- **Lignes affichées** : dès qu'un local est sélectionné, une ligne est créée (une seule fois, aux affichages suivants la ligne déjà créée est réutilisée) pour chaque **bien actif actuellement affecté à ce local** dans l'inventaire, avec :
  - **Bien ID** et **Désignation** : toujours lus en direct depuis l'Inventaire (jamais dupliqués ici), donc toujours à jour même si ces informations changent par ailleurs.
  - **Type de bien** (liste déroulante, alimentée par `/admin/destinations`, onglet Types de bien, §2.12), **Commentaire** (texte libre) et **Statut** (liste déroulante : **Présent**, **Absent**, **En Trop**) : modifiables à tout moment (`POST /inventaire-local/lines/{id}/update`) et enregistrés immédiatement au clic sur le bouton d'enregistrement (icône disquette) de la ligne. La valeur déjà connue, le cas échéant, est présélectionnée. Statut « Présent » par défaut. Pour un bien connu de l'inventaire, le Type de bien modifié ici est **le même champ** que sur l'Inventaire (§2.4, `Asset.type_bien_id`) — une seule valeur partagée, modifiable indifféremment depuis l'une ou l'autre page ; pour un bien « en trop », il est propre à la ligne.
  - **Bien ID coloré au clic sur l'icône disquette** : vert si le statut enregistré est **Présent**, orange si **En Trop**, rouge si **Absent** ; couleur de police normale tant que la ligne n'a jamais été **explicitement enregistrée** (création automatique par défaut à l'affichage du local non comprise). Un bien « en trop » est coloré dès son ajout (action explicite). Même règle, même coloration, sur l'Inventaire (§2.4).
- **Ajouter un bien en trop** (`POST /inventaire-local/add`) : pour un bien physiquement présent dans le local mais non affecté à celui-ci dans l'inventaire (ou totalement inconnu) — Bien ID, Désignation, Type de bien et commentaire (tous facultatifs). Statut toujours **Présent** à l'ajout (le bien vient d'être constaté sur place) ; modifiable ensuite comme les autres lignes. Repéré dans le tableau par un badge « En trop » à côté du Bien ID. Seule une ligne « en trop » peut être supprimée (`POST /inventaire-local/lines/{id}/delete`) — une ligne liée à un bien connu de l'inventaire ne peut pas l'être (elle serait de toute façon recréée au prochain affichage du local tant que le bien y reste affecté).
  - **Bien sans numéro** (ex. fauteuil ou meuble non étiqueté, trouvé lors du contrôle) : le Bien ID peut être laissé vide — une référence temporaire est alors générée automatiquement (`SN-<id>`, `InventoryCheckService.add_extra_line`), repérée par un badge **« Sans numéro »** à la place du badge « En trop ». La Désignation permet de décrire l'objet en attendant. Une fois son vrai Bien ID retrouvé (sur place ou dans le logiciel de gestion d'inventaire externe), l'action **Rattacher à un Bien ID** (icône chaîne, `POST /inventaire-local/lines/{id}/attach-bien-id`, visible uniquement sur les lignes « Sans numéro », **confirmation obligatoire** pour s'assurer que ce Bien ID a bien été reporté dans l'application d'inventaire) le renseigne sans recréer la ligne (commentaire et statut conservés) :
    - si ce Bien ID correspond à un bien actif déjà connu de l'inventaire, la ligne devient une ligne **normale** liée à cet Asset (Bien ID, désignation et type de bien alors lus en direct dessus, comme pour toute ligne connue) — refusé (400) si ce bien a déjà sa propre ligne dans ce même local (éviterait un doublon) ;
    - sinon, la référence temporaire est simplement remplacée par le Bien ID indiqué : la ligne reste « en trop » (toujours à reporter dans le logiciel de gestion d'inventaire externe), mais n'est plus « sans numéro ».
- **Export** :
  - **Exporter en CSV** (`GET /inventaire-local/export-csv?local=...`) : CSV (`;`, avec en-tête) des lignes du local sélectionné — Bien ID, **Bien ID confirmé** (Non tant qu'une référence temporaire « Sans numéro » n'a pas été rattachée), Désignation, Type de bien, Commentaire, Statut.
...
- **Exporter en CSV** (`GET /inventaire-local/export-csv-statut?statut=Absent|En Trop`) : CSV (`;`, avec en-tête) des biens au statut sélectionné, tous locaux confondus — Local, Bien ID, **Bien ID confirmé**, Désignation, Type de bien, Commentaire, Statut. La colonne **Bien ID confirmé** vaut « Non » pour une référence temporaire « Sans numéro » pas encore rattachée (voir ci-dessus) : à ne pas reporter telle quelle dans le logiciel de gestion d'inventaire externe tant qu'elle n'est pas passée à « Oui ».
- **Rattacher à un Bien ID** : même action que sur l'onglet Par local (icône chaîne), disponible directement depuis cet onglet pour les lignes « Sans numéro ».
...
| `inventory_check_lines` | Lignes du suivi de l'inventaire par local (§2.13) | `id`, `local_libelle`, `asset_id` (FK → `assets`, nullable — NULL pour un bien "en trop"), `bien_id`/`designation`/`type_bien_id` (renseignés uniquement si `asset_id` est NULL), `bien_id_temporaire` (True si `bien_id` est une référence générée automatiquement "SN-\<id\>" en attendant rattachement, voir §2.13), `commentaire`, `statut` (`Présent`/`Absent`/`En Trop`), `updated_by`, `updated_at` |
...
| `POST` | `/api/print/jobs/{job_id}/generate` | gestionnaire+ | Génère un fichier `.cmd` par bien du lot, déposés directement dans `generated/` (racine, pas de sous-dossier) sur le disque du serveur — **point d'intégration avec le logiciel/matériel d'impression**, aucun téléchargement via l'API |
