...
@router.post("/glpi-locations/assets/{asset_id}/exclude")
def glpi_locations_exclude_asset(
    asset_id: int,
    current_user=Depends(get_current_user_web),
    db: Session = Depends(get_db)
):
    """
    Marque un bien comme exclu (is_active = False) directement depuis
    le tableau des écarts de codes lieux.
    """
    ...
    logger.warning(
        "Bien %s exclu depuis la page codes lieux par %s",
        asset.bien_id,
        current_user["sub"]
    )

    return RedirectResponse(
        url="/glpi-locations?excluded=1",
        status_code=303
    )
